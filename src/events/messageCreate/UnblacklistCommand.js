...
async (client, message) => {
    ...
    try {
        if (subCommand === 'user') {
            ...
            await BlacklistUserSchema.deleteOne({ userID: targetId });
            return replyAndLog("Green", `✅ User <@${targetId}> has been unblacklisted.`, "Unblacklist User");
        }

        if (subCommand === 'guild') {
            const existing = await BlacklistGuildSchema.findOne({ guildID: targetId });
            if (!existing) {
                return message.reply(`❗ This guild is not blacklisted.`);
            }

            await BlacklistGuildSchema.deleteOne({ guildID: targetId });
            return replyAndLog("Green", `✅ Guild \`${targetId}\` has been unblacklisted.`, "Unblacklist Guild");
        }
    } catch (error) {
        console.error(`Error processing unblacklist command:`, error);
        return message.reply("❗ There was an error while processing the unblacklist.");
    }
};
